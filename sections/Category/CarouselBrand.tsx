...
function Card({ image, label, link, }: Brand) {
    return (<a className="flex flex-col group" href={link}>
      <Image class="rounded-full" src={image ?? "https://placehold.co/160x160"} alt={label} width={160} height={160} loading="lazy"/>
    </a>);
}
    ...
    return (<div id={id}>
      ...
      <div id="fakeLoading" class="container px-5 flex gap-5 overflow-x-hidden">
        {new Array(15).fill("").map((_, index) => (<div key={index}>
            <div class="rounded-full skeleton" style={{
                width: "160px",
                height: "160px",
            }}/>
          </div>))}
      ...
      <div id="content" class="hidden container px-0 lg:px-10 overflow-hidden relative">
        <div class="overflow-hidden">
          <div class="swiper-wrapper">
            {brands?.map((item, index) => (<div class="swiper-slide max-w-[160px] sm:max-w-[160px] first:ml-5 last:mr-5 lg:first:ml-0 lg:last:mr-0" key={index}>
                <Card {...item}/>
              </div>))}
          </div>
