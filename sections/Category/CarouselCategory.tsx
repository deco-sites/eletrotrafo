...
function Card({ image, label = "Categoria", link, }: Category) {
    return (<a className="flex flex-col group" href={link}>
      <Image className="rounded-xl mb-2" src={image ?? "https://placehold.co/114x106"} alt={label} width={114} height={106} loading="lazy"/>
...
const CarouselCategory = ({ categories, title }: Props) => {
    ...
    return (<div id={id}>
      ...
      <div id="fakeLoading" class="container px-5 flex gap-5 overflow-x-hidden">
        {new Array(15).fill("").map((_, index) => (<div key={index}>
            <div class="rounded-xl mb-2 skeleton" style={{
                width: "114px",
                height: "106px",
            }}/>
      ...
      <div id="content" class="hidden container px-0 lg:px-10 overflow-hidden relative">
        <div class="overflow-hidden">
          <div class="swiper-wrapper">
            {categories?.map((item, index) => (<div class="swiper-slide max-w-[114px] first:ml-5 last:mr-5 lg:first:ml-0 lg:last:mr-0 cursor-pointer" key={index}>
                <Card {...item}/>
              </div>))}
          </div>
