...
export default function Searchbar({ placeholder = "O que deseja?", loader, searchBarDrawer }: SearchbarProps) {
    const slot = useId();
    return (<div class="search-bar-container w-full lg:w-[300px] lg:hover:w-[50vw] flex justify-end">
      <div class={`search-bar-wrapper w-full relative lg:max-w-[300px] lg:hover:max-w-[50vw] ${searchBarDrawer ? "gap-[unset]" : ""}`}>
      ...
      >
        ...
        {searchBarDrawer
          ...
          <form id={SEARCHBAR_INPUT_FORM_ID} action={ACTION} class={`join flex gap-[20px] lg:gap-0 ${searchBarDrawer ? "my-5 w-full px-5" : ""}`}>
            <button type="submit" form={SEARCHBAR_INPUT_FORM_ID} class="bg-transparent border-none lg:absolute lg:-left-10 lg:top-1/2 lg:-translate-y-1/2" aria-label="Search" tabIndex={-1}>
              {searchBarDrawer
            ? <Icon id="search-drawer" class="inline"/>
            : <Icon id="search" class="inline"/>}
            </button>
            <input type="text" name={NAME} class={`rounded-[30px] outline-none py-[8.5px] px-5 placeholder-middle-gray w-full lg:w-auto`} placeholder={placeholder} autocomplete="off" data-hx-target={`#${slot}`} data-hx-post={loader &&
        ...
        {/* Suggestions slot */}
        <div class="suggestions-wrapper lg:absolute right-0 lg:bg-white px-5 pt-0 lg:py-8" id={slot}/>
